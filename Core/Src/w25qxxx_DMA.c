...
static volatile  UINT  WriteStatus = 0, ReadStatus = 0, WriteReadStatus = 0;
...
uint8_t W25Qxxx_ReadBytes(uint8_t *pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead)
{
	...
	if (//HAL_SPI_Receive(&hspi_flash, pBuffer, NumByteToRead, SPI_FLASH_TIMEOUT) != HAL_OK
			            W25Qxxx_SPI_Rx(pBuffer, NumByteToRead) != HAL_OK)
	{
		W25Qxxx_SPI_Rx(pBuffer, NumByteToRead);
		W25Qxxx_Disable();
		return 1;
	}
...
/**
 * @brief spi flash W25Qxxx disk io driver
 */
#if 	EN_SPI_FLASH_DISK_IO
...
/**
 * @brief DMA callback function implementation
 */
